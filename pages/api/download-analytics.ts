...
function generateAnalytics(stats: DownloadStats): AnalyticsResponse {
  ...
  const weekRecords = stats.records.filter(
    (r) => new Date(r.timestamp) >= week
  );
  const monthRecords = stats.records.filter(
    (r) => new Date(r.timestamp) >= month
  );
  ...
  // Browser stats
  const browserStats = stats.records.reduce((acc, record) => {
    acc[record.device.browser] = (acc[record.device.browser] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);

  // OS stats
  const osStats = stats.records.reduce((acc, record) => {
